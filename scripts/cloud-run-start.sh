...
# Vertex AI + Roboflow calls every 10 seconds the whole time). Run
# scripts/cloud-run-stop.sh when you're done. See docs/RUNBOOK.md#google-cloud-run
# for the full explanation, including the IAM setup this depends on.
set -euo pipefail
...
if [ ! -f .env ]; then
  echo "Error: .env not found. Create it with ROBOFLOW_API_KEY=... first (see docs/RUNBOOK.md#setting-up-the-roboflow-key)." >&2
  exit 1
fi
